...
fn main() {
    App::default()
        ...
        //This system requests the loading of the script file and spawns an entity.
        .add_startup_system(startup.system())
        //If we didn't add an engine resource yet(this is checked by the run criteria),
        //we should check if the scripts we requested were loaded.
        .add_system(
            wait_for_load
                .system()
                .with_run_criteria(added_engine_res_run_criteria.system()),
        )
        //This system increments the component B for our entities from inside a script.
...
///This component has a function attached to it, because we want to be able to call this function from scripts,
///we need to both register the type B in the engine, and its functions we want to expose to rhai.
#[derive(Debug, Clone)]
...
fn added_engine_res_run_criteria(engine_res: Option<Res<ScriptingEngine>>) -> ShouldRun {
    //If we already added the engine to the resources, we shouldn't run wait_for_load anymore.
    if engine_res.is_some() {
        ShouldRun::No
    }
    //We didn't add the resource yet, we need to check if the script was loaded again.
    else {
        ShouldRun::Yes
    }
}
...
fn wait_for_load(
    ...
    if let Some(script) = scripts.get(handle.id) {
        ...
        //Store it in a resource for further use
        commands.insert_resource(ScriptingEngine(engine, ast));
    }
}
//Every frame this system executes the script we loaded.
